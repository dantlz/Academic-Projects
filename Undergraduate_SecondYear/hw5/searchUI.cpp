...
SearchUI::SearchUI() : QWidget(NULL){
    ...
    mainList_nameSort_button->setChecked(true);
    ...
    main_layout->addWidget(mainListSort_group,4,0,1,3);
    ...
    main_layout->addWidget(mainList, 6,0,8,3);
    ...
    QObject::connect(searchInput, SIGNAL(returnKey_pressed()), this, SLOT(returnKey_hit()));
...
SearchUI::~SearchUI(){
    ...
    for(set<WebPage*>::iterator it = search_result.begin(); it != search_result.end(); ++it){
    	delete *it;
    }
    mainList_data.clear();
    inList_data.clear();
...
void SearchUI::populate_mainList(){
    ...
    //Put the search result into an vector
    for(set<WebPage*>::iterator it = search_result.begin(); it != search_result.end(); ++it){
        mainList_data.push_back(*it);
    }

    //Sort it based on the current sort selection
    if(mainList_sortType->checkedButton()==mainList_nameSort_button)
    ...
    //Populate the list
    for(unsigned int i=0;i<mainList_data.size();i++){
        ListItem* item= new ListItem(mainList_data[i]);
...
void SearchUI::populate_inList(WebPage* wp){
    ...
    if(wp){
        MySetWebPage incoming = wp->incoming_links();
        inList_data.clear();
...
void SearchUI::populate_outList(WebPage* wp){
    ...
    if(wp){
        MySetWebPage outgoing = wp->outgoing_links();
        outList_data.clear();
...
///Search a single input, the entire string
void SearchUI::search_single(string input){
    search_result.clear();
    search_result = sn.findWord(input);
}

//Search for pages that contain all words in the vector
void SearchUI::search_and(vector<string> input_words){
    search_result.clear();
...
//search for all pages that contain any of words in the vector
void SearchUI::search_or(vector<string> input_words){
    search_result.clear();
    search_result = sn.findWord(input_words[0]);
    for(unsigned int i=1;i<input_words.size();i++){
...
//Sort by file name
void SearchUI::sort_name(vector<WebPage*>& v){
     webpage_filename_comp comp;
     mergeSort(v, comp);
}
//Sort by the number of incoming links
void SearchUI::sort_inLink(vector<WebPage*>& v){
     webpage_inLink_comp comp;
     mergeSort(v, comp);
}
//Sort by the number of outgoing links
void SearchUI::sort_outLink(vector<WebPage*>& v){
     webpage_outLink_comp comp;
     mergeSort(v, comp);

}
//Convert a string's alphabets to all lower case for standardization.
string SearchUI::lowerCase(string s){
    unsigned int i=0;
    while (i<s.size()){
        if(isalpha(s[i]))
            s[i]=tolower(s[i]);
