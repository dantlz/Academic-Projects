...
public class FactorySimulation {
	...
	FactorySimulation(Factory inFactory, JTable inTable) {
		...
		for(Resource resource : mFactory.getResources()) {
			FactoryResource fr;
			FactoryToolbox toolbox;
			if(resource.getName().equals("Toolbox")){
				toolbox = new FactoryToolbox(new Rectangle(resource.getX(), resource.getY(), 1,1), resource.getQuantity());
				mFObjects.add(toolbox);
				mFNodes[toolbox.getX()][toolbox.getY()].setObject(toolbox);
				mFNodeMap.put("Toolbox", mFNodes[toolbox.getX()][toolbox.getY()]);
			}
			else{
